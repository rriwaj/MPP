package labwo.lesson9.problems.prob3;
...
public class EmployeeInfoBetter {
	...
	Map<SortMethod, Comparator<Employee>> map = new HashMap<SortMethod, Comparator<Employee>>() {
		{
			put(SortMethod.BYNAME,
					Comparator.comparing(byName).thenComparing(bySalary)
							.reversed());
			put(SortMethod.BYSALARY, Comparator.comparing(bySalary)
	...
	public void sort(List<Employee> emps, final SortMethod method) {
		...
		// .reversed());
		// } else {
		// Collections.sort(emps, Comparator.comparing(bySalary)
		// .thenComparing(byName));
		// }
